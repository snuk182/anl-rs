use super::noise_gen::{
	cellular_function_2d,
	cellular_function_3d,
	cellular_function_4d,
	cellular_function_6d,
};
...
impl CellularGenerator {
	...
	pub fn get_4d<'a>(&'a mut self, x: f64, y: f64, z: f64, w: f64) -> &'a CellularCache {
	    if !self.cache4.valid || x!=self.cache4.x || y!=self.cache4.y || z!=self.cache4.z || w!=self.cache4.w {
	        cellular_function_4d(x,y,z,w, self.seed, &mut self.cache4.f, &mut self.cache4.d);
	        self.cache4.x=x;
	        self.cache4.y=y;
	        self.cache4.z=z;
	        self.cache4.w=w;
	...
	pub fn get_6d<'a>(&'a mut self, x: f64, y: f64, z: f64, w: f64, u: f64, v: f64) -> &'a CellularCache {
	    if self.cache6.valid || x!=self.cache6.x || y!=self.cache6.y || z!=self.cache6.z || w!=self.cache6.w || u!=self.cache6.u || v!=self.cache6.v {
	        cellular_function_6d(x,y,z,w,u,v, self.seed, &mut self.cache6.f, &mut self.cache6.d);
	        self.cache6.x=x;
	        self.cache6.y=y;
	        self.cache6.z=z;
	        self.cache6.w=w;
