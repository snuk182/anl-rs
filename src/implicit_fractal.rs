...
impl ImplicitFractal {
	...
	pub fn decarpentier_swiss_get_2d(&mut self, x: f64, y: f64) -> f64 {
		...
		for i in 0..self.numoctaves as usize {
			if let Some(ref ii) = self.source[i] {
				...
				let dx = get_dx_2(&mut *bii, x+self.offset*dx_sum, y+self.offset*dy_sum);
				let dy = get_dy_2(&mut *bii, x+self.offset*dx_sum, y+self.offset*dy_sum);
				sum += amp * (1.0 - n.abs());
				dx_sum += amp * dx * -n;
				dy_sum += amp * dy * -n;
	...
	pub fn decarpentier_swiss_get_4d(&mut self, x: f64, y: f64, z: f64, w: f64) -> f64 {
		...
		for i in 0..self.numoctaves as usize {
			if let Some(ref ii) = self.source[i] {
				let mut bii = ii.borrow_mut();
				let n = bii.get_4d(x+self.offset*dx_sum, y+self.offset*dy_sum, z+self.offset*dz_sum, w+self.offset*dw_sum);
				let dx = get_dx_4(&mut *bii, x+self.offset*dx_sum, y+self.offset*dy_sum, z+self.offset*dz_sum, w+self.offset*dw_sum);
				let dy = get_dy_4(&mut *bii, x+self.offset*dx_sum, y+self.offset*dy_sum, z+self.offset*dz_sum, w+self.offset*dw_sum);
				let dz = get_dz_4(&mut *bii, x+self.offset*dx_sum, y+self.offset*dy_sum, z+self.offset*dz_sum, w+self.offset*dw_sum);
				let dw = get_dw_4(&mut *bii, x+self.offset*dx_sum, y+self.offset*dy_sum, z+self.offset*dz_sum, w+self.offset*dw_sum);
				sum += amp * (1.0 - n.abs());
				dx_sum += amp * dx * -n;
				dy_sum += amp * dy * -n;
				dz_sum += amp * dz * -n;
	...
	pub fn decarpentier_swiss_get_3d(&mut self, x: f64, y: f64, z: f64) -> f64 {
		...
		for i in 0..self.numoctaves as usize {
			if let Some(ref ii) = self.source[i] {
				let mut bii = ii.borrow_mut();
				let n = bii.get_3d(x+self.offset*dx_sum, y+self.offset*dy_sum, z+self.offset*dz_sum);
				let dx = get_dx_3(&mut *bii, x+self.offset*dx_sum, y+self.offset*dy_sum, z+self.offset*dz_sum);
				let dy = get_dy_3(&mut *bii, x+self.offset*dx_sum, y+self.offset*dy_sum, z+self.offset*dz_sum);
				let dz = get_dz_3(&mut *bii, x+self.offset*dx_sum, y+self.offset*dy_sum, z+self.offset*dz_sum);
				sum += amp * (1.0 - n.abs());
				dx_sum += amp * dx * -n;
				dy_sum += amp * dy * -n;
				dz_sum += amp * dz * -n;
	...
	pub fn decarpentier_swiss_get_6d(&mut self, x: f64, y: f64, z: f64, w: f64, u: f64, v: f64) -> f64 {
		...
		for i in 0..self.numoctaves as usize {
			if let Some(ref ii) = self.source[i] {
				let mut bii = ii.borrow_mut();
				let n = bii.get_6d(x+self.offset*dx_sum, y+self.offset*dy_sum, z+self.offset*dz_sum, w+self.offset*dw_sum, u+self.offset*du_sum, v+self.offset*dv_sum);
				let dx = get_dx_6(&mut *bii, x+self.offset*dx_sum, y+self.offset*dy_sum, z+self.offset*dx_sum, w+self.offset*dw_sum, u+self.offset*du_sum, v+self.offset*dv_sum);
				let dy = get_dy_6(&mut *bii, x+self.offset*dx_sum, y+self.offset*dy_sum, z+self.offset*dz_sum, w+self.offset*dw_sum, u+self.offset*du_sum, v+self.offset*dv_sum);
				let dz = get_dz_6(&mut *bii, x+self.offset*dx_sum, y+self.offset*dy_sum, z+self.offset*dz_sum, w+self.offset*dw_sum, u+self.offset*du_sum, v+self.offset*dv_sum);
				let dw = get_dw_6(&mut *bii, x+self.offset*dx_sum, y+self.offset*dy_sum, z+self.offset*dz_sum, w+self.offset*dw_sum, u+self.offset*du_sum, v+self.offset*dv_sum);
				let du = get_du_6(&mut *bii, x+self.offset*dx_sum, y+self.offset*dy_sum, z+self.offset*dz_sum, w+self.offset*dw_sum, u+self.offset*du_sum, v+self.offset*dv_sum);
				let dv = get_dv_6(&mut *bii, x+self.offset*dx_sum, y+self.offset*dy_sum, z+self.offset*dz_sum, w+self.offset*dw_sum, u+self.offset*du_sum, v+self.offset*dv_sum);
				sum += amp * (1.0 - n.abs());
				dx_sum += amp * dx * -n;
				dy_sum += amp * dy * -n;
				dz_sum += amp * dz * -n;
...
fn min<'a, T>(a: &'a T, b: &'a T) -> &'a T where T: ::std::cmp::PartialOrd {
	match a.partial_cmp(b).unwrap() {
		::std::cmp::Ordering::Equal | ::std::cmp::Ordering::Less => a,
		::std::cmp::Ordering::Greater => b
	}
}

fn max<'a, T>(a: &'a T, b: &'a T) -> &'a T where T: ::std::cmp::PartialOrd {
	match a.partial_cmp(b).unwrap() {
